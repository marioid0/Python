...
#chatgpt:
#   def is_valid_url 100%
#   clear_dns (botei no gpt por que não estava limpando o cache DNS)
#   def unlock_site => 20% (dificuldades em apagar a linha correta)

# Caminho para o arquivo hosts no Windows
hosts_path = r"C:\Windows\System32\drivers\etc\hosts"
redirect_ip = "127.0.0.1"

# Função para validar se a entrada é um site tradicional (URL)
            ...
            try:
                with open(hosts_path, "r+") as file:
                    ...
                    if base_entry not in content and www_entry not in content:
                        ...
                        clear_dns_cache()  # Limpa o cache de DNS após bloquear
                    else:
...
def animate_button(button):
    ...
    button.config(background="#f54291")  # Cor vibrante
    button.after(100, lambda: button.config(background=original_color))

# Carregar fonte personalizada
try:
    pixel_font = ("Minecraftia", 14)  # Fonte Minecraftia, caso tenha sido instalada
except:
...
# Lista de sites bloqueados com visual moderno
...
# Adicionando sites bloqueados ao iniciar
